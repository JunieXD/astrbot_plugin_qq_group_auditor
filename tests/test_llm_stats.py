...
from qq_group_auditor.llm_stats import UsageStore, format_statistics, summarize
from qq_group_auditor.llm_usage import normalize_statistics


def begin(store, review_id="r1", attempt=1, **overrides):
    fields = dict(review_id=review_id, attempt=attempt, application_id=123,
                  platform_id="onebot", group_id="123", source="plugin", provider_id="p",
                  model="configured", model_source="configured", system_prompt="固定规则", prompt="申请答案")
    fields.update(overrides)
    return store.begin(**fields)


def response(tokens=None, text='{"approve":true,"reason":"OK"}'):
    return Obj(completion_text=text, id="response-id", raw_completion={
        "model": "actual-model", "id": "response-id", "choices": [{"finish_reason": "stop"}],
        "usage": tokens if tokens is not None else {"prompt_tokens": 100, "completion_tokens": 20,
                       "prompt_tokens_details": {"cached_tokens": 80}},
    })


def store_config(**kwargs):
    return normalize_statistics({"prices": [{"model": "actual-model", "cached_input_per_million": "1",
                      "uncached_input_per_million": "4", "output_per_million": "8"}], **kwargs})
...
def test_crash_and_cancel_remain_unknown_and_do_not_synthesize_latency(tmp_path):
    ...
    begin(store, review_id="crash")
    cancelled = begin(store, review_id="cancelled")
...
def test_disabled_recording_still_allows_historical_queries():
    ...
    assert begin(store, review_id="disabled") is None
...
def test_filters_scope_time_model_and_application():
    store = UsageStore(":memory:", store_config())
    begin(store)
    begin(store, review_id="different-group", group_id="456")
    assert len(store.query(group_ids=["123"], model="configured", application_id=123)) == 1
    assert store.query(group_ids=["123"], model="other") == []
    assert store.query(group_ids=["123"], application_id=456) == []
...
def test_retention_prunes_finished_records_but_preserves_pending(monkeypatch):
    ...
    finished = begin(store, review_id="finished")
    store.finish(finished, status="provider_error", duration_ms=1, error_type="TimeoutError")
    begin(store, review_id="pending")
    later = time.time() + 2 * 86400
    monkeypatch.setattr("qq_group_auditor.llm_stats.time.time", lambda: later)
    store._next_prune = 0
    begin(store, review_id="new")
    assert [r["review_id"] for r in store.query(group_ids=["123"])] == ["pending", "new"]
...
def test_report_reader_does_not_block_live_writes(tmp_path):
    ...
    with store._reader() as reader:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            call_id = pool.submit(begin, store, review_id="during-report").result(timeout=2)

...
def extract_usage(response: Any) -> dict:
    ...
    if raw is not None:
        ...
        if total is not None:
            if (hit is not None and hit > total) or (miss is not None and miss > total) or (
                hit is not None and miss is not None and hit + miss != total
            ):
        ...
        if reasoning is not None and output is not None and reasoning > output:
            ...
            reasoning = None
        result.update(input_tokens=total, cached_tokens=hit, uncached_tokens=miss,
    ...
    if result["usage_source"] == "unknown" and not openai_shape and normalized is not None:
        ...
        if any(v not in (None, 0) for v in (other, cached, output)):
            ...
            result.update(input_tokens=total, cached_tokens=cached, uncached_tokens=other,
                          output_tokens=output, usage_source="astrbot")

...
class ActionGuard:
    ...
    def _next_ticket(self, account):
        now = time.time()
        ready = [ticket for ticket in self._pending[account]
                 if ticket["ready_at"] <= now]
        if not ready:
            return None
        ...
        def order(ticket):
            ...
            return (max(0, ticket["priority"] - aging),
                    served.get(ticket["group"], 0), ticket["sequence"])
    ...
    async def run(self, *, account, online, action, config, delay=(0, 0),
        ...
        self._sequence += 1
        ticket = dict(ready_at=ready_at, priority=max(0, int(priority)),
                      group=str(group or "其他"), sequence=self._sequence)
        self._pending.setdefault(account, []).append(ticket)
        self._wake_queue(account)
        logger.info("QQ 操作入队：账号=%s，任务=%s，群=%s，随机等待=%.1f 秒，最早执行时间=%s",
    ...
    async def _run_ticket(self, *, ticket, enqueued_at, label, account, online,
        ...
        while True:
            ...
            now = time.time()
            if deadline() > now:
                waiting("账号恢复等待" if state.get("recovery_until", 0) == deadline()
                        else "随机延迟" if ready_at == deadline() else "账号操作间隔")

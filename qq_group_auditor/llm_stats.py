...
class UsageStore:
        ...
        try:
            ...
            self._db.executescript("""
                CREATE TABLE IF NOT EXISTS llm_calls (
                    ...
                    review_id TEXT NOT NULL, application_id INTEGER,
        ...
        with self._lock:
            ...
            try:
                ...
                if time.monotonic() - self._last_warning >= 60:
                    logger.warning("LLM 统计写入失败，审核继续：%s", type(exc).__name__)
    ...
    def _prune(self) -> None:
        ...
        if days:
            with self._db:
                self._db.execute("DELETE FROM llm_calls WHERE started_at < ? AND status != 'pending'",
    ...
    def _reader(self):
        ...
        # the write lock while live reviews are saving their counters.
        if self.database == ":memory:":
            with self._lock:
                yield self._db
        else:
    ...
    def begin(self, *, system_prompt: str, prompt: str, **metadata) -> int | None:
        ...
        def write():
            ...
            if self.config["store_content"]:
                ...
                values.update(system_prompt_text=system_prompt[:maximum], prompt_text=prompt[:maximum],
                              content_truncated=int(len(system_prompt) > maximum or len(prompt) > maximum))
            columns = ",".join(values)
            with self._db:
                cursor = self._db.execute(f"INSERT INTO llm_calls ({columns}) VALUES ({','.join('?' for _ in values)})",
                                          tuple(values.values()))
            return cursor.lastrowid
        return self._safe(write)

    def finish(self, call_id: int | None, *, status: str, duration_ms: float,
               response: Any = None, error_type: str = "") -> None:
        if call_id is None:
            return
        def write():
            ...
            with self._db:
                self._db.execute(f"UPDATE llm_calls SET {','.join(k+'=?' for k in values)} WHERE id=?",
                                 (*values.values(), call_id))
        self._safe(write)

    def parsed(self, call_id: int | None, status: str) -> None:
        if call_id is None or status not in {"success", "invalid_json"}:
            return
        def write():
            with self._db:
                self._db.execute("UPDATE llm_calls SET status=? WHERE id=? AND status='returned'", (status, call_id))
        self._safe(write)

    def query(self, *, group_ids: list[str], since: float = 0, until: float | None = None,
              model: str | None = None, application_id: int | None = None,
              content: bool = False) -> list[dict]:
        if not group_ids:
            return []
        ...
        with self._reader() as reader:
            rows = reader.execute(f"SELECT {columns} FROM llm_calls WHERE {' AND '.join(clauses)} ORDER BY started_at,id LIMIT 100001", params).fetchall()
        if len(rows) > 100000:
    ...
    def export_csv(self, rows: list[dict], directory: Path) -> Path:
        ...
        try:
            with self._reader() as reader, path.open("x", newline="", encoding="utf-8-sig") as f:
                ...
                writer.writeheader()
                for row in rows:
                    stored = reader.execute("SELECT * FROM llm_calls WHERE id=? AND group_id=?",
                                            (row["id"], row["group_id"])).fetchone()
                    if stored is None:
                        continue
                    values = dict(stored)
                    for key in ("started_at", "finished_at"):
                        values[key + "_utc"] = datetime.fromtimestamp(values[key], timezone.utc).isoformat() if values[key] is not None else ""
...
def format_statistics(rows: list[dict], *, days: int, enabled: bool) -> str:
    ...
    latency = "未知" if stats["average_ms"] is None else f"{stats['average_ms']/1000:.2f}s / {stats['p95_ms']/1000:.2f}s"
    ...
    lines = [f"LLM 统计 · 最近 {days} 天（仅你管理的群）",
             f"调用 {count} 次；审核批次 {stats['reviews']}；重试 {stats['retries']} 次",
             "状态：" + "、".join(f"{STATUS_LABELS.get(k, k)}={v}" for k, v in sorted(stats["statuses"].items())),
             f"输入：{tokens('input_tokens')}", f"缓存命中：{tokens('cached_tokens')}",
             f"缓存未命中：{tokens('uncached_tokens')}", f"输出：{tokens('output_tokens')}",
    ...
    lines.append("包含测试及无效输出；不含审批随机等待。缺失用量和内部网络重试可能使费用不完整。")
    if not enabled:

...
已经有成功名片操作记录的成员不会重复处理，命令可安全重复执行。由于未读取当前群名片，平台返回的设置失败可能表示成员已退群、Bot 权限不足或其他 NapCat 错误，汇总结果会统一列在“设置接口失败”中。历史记录没有真实入群通知时，`{join_date}` 和 `{join_time}` 使用审批时间作为近似值；昵称优先使用申请快照，缺失时通过独立的 `get_stranger_info` 获取，仍然失败才回退到 QQ 号。

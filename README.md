...
命令会一次获取当前群成员，优先按 QQ 号和入群时间关联申请。早期 NapCat 系统消息没有提供真实申请时间时，只有该 QQ 恰好存在一个可用申请候选才会降级使用；存在多次且无法可靠区分时仍会跳过。已经有成功名片操作记录的成员不会重复处理；当前名片已经等于模板结果时只补记成功，不调用修改接口；已退群、被踢、平台报告不可修改、答案缺失或时间无法可靠匹配的记录会分别计数并跳过。命令可安全重复执行，最终会返回成功、已处理、跳过、失败和不在群内等汇总数字。
